...
import RangeDate from './RangePicker/RangeDate';
import RangeTime from './RangePicker/RangeTime';
import './react-datetime-range.css';
...
const CalendarPickerComponent: React.FC<CalendarPickerComponentProps> = memo(
  ({
    ...
    const componentClass = useMemo(() => cx('react-datetime-range', internalShow && 'visible'), [internalShow]);
    const LOCALE_DATA: IObjectKeysAny = useMemo(() => (LOCALE[locale] ? LOCALE[locale] : LOCALE['en-us']), [locale]);
    return (
      <div className={componentClass}>
        <svg className="react-datetime-range__close" viewBox="0 0 20 20" width="15" height="15" onClick={handleOnClose}>
        ...
        <div className={`react-minimal-datetime-date-piker`}>
          <div className={`react-datetime-range__calendar`}>
            <Calendar
              defaultDate={defaultDate}
              locale={locale}
          ...
          {type === TYPES[1] && (
            <div className="react-datetime-range__time-piker" style={{ marginTop: '10px' }}>
              {/* <RangeTime
        ...
        {enableTimeSelection && (
          <div
            className={cx('react-datetime-range__button', 'react-datetime-range__button--type', !selected && 'disabled')}
